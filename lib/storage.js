...
var BLOCK_RECEIVED = 2
function noop () {}

// state + buffer
//
// specific implementations (such as FSStorage) are permitted to steal
// the buffer in state BLOCK_RECEIVED
function Block (state, buffer) {
  this.state = state || BLOCK_UNKNOWN
  this.buffer = buffer
  this.hashed = false
  this.stored = false
}

Block.prototype.receiveBuffer = function (buffer) {
  this.state = BLOCK_RECEIVED
  this.buffer = buffer
}
...
function Piece (index, hash, buffer, noVerify) {
  ...
  if (typeof buffer === 'number') {
    // alloc buffer lazily
    self.length = buffer
    for(offset = 0; offset < self.length; offset += BLOCK_LENGTH) {
      self.blocks.push(new Block())
    }
  } else {
    // use buffer provided
    self.buffer = buffer
    ...
    for(offset = 0; offset < self.length; offset += BLOCK_LENGTH) {
      self.blocks.push(new Block(BLOCK_UNKNOWN, self.buffer.slice(offset, offset + BLOCK_LENGTH)))
    }
  }
...
Piece.prototype.readBlock = function (offset, length, cb) {
  ...
  var blockIndex = Math.floor(offset / BLOCK_LENGTH)
  var block = self.block[blockIndex]
  
  if (block && block.state === BLOCK_RECEIVED && block.buffer) {
    var buffer = block.buffer
    var blockOffset = blockIndex * BLOCK_LENGTH
    if (offset > blockOffset)
      buffer = buffer.slice(offset - blockOffset)
...
Piece.prototype.writeBlock = function (offset, buffer, cb) {
  ...
  var blockIndex = Math.floor(offset / BLOCK_LENGTH)
  if (blockIndex * BLOCK_LENGTH - offset !== 0)
    return cb(new Error('write to non-aligned block: ' + offset))
  var block = self.blocks[blockIndex]
  if (!block)
    return cb(new Error('invalid block offset: ' + offset))
  
  if (block.state === BLOCK_RECEIVED) {
    debug('double write to piece ' + self.piece + ' at ' + offset + '+' + buffer.length)
    return cb(null)
  }

  debug('piece ' + self.index + ' writeBlock@' + offset + '+' + buffer.length)
  block.receiveBuffer(buffer)
...
Piece.prototype.reserveBlock = function (endGame) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if ((!endGame && block.state === BLOCK_RESERVED) || block.state === BLOCK_RECEIVED) {
      continue
    }
    block.state = BLOCK_RESERVED
    debug('reserving ' + i * BLOCK_LENGTH + '+' + ((i === len - 1)
...
Piece.prototype._hashNext = function () {
  ...
  var block = this.blocks[this.blocksHashed]
  if (block && block.state === BLOCK_RECEIVED) {
    debug('piece ' + this.index + ' _hashNext: write')
    this.blocksHashed += 1
    if (this.sha.write(block.buffer)) {
...
Storage.prototype.read = function (index, range, cb, force) {
  ...
  cb = (function (cb) {
    return function (err, res) {
      debug("read " + index + " cb: " + (err && err.stack) + " " + (res && res.length))
      cb(err, res)
    }
  })(dezalgo(cb))
  ...
  function readNextBlock () {
    debug("piece " + index + " read " + blocks.map(function(b){return b.length}).join("+") + " blocks, " + length + " to go")
