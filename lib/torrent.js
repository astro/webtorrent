    ...
    for (var i = self._selections.length; i--;) {
      ...
      if (self.strategy === 'rarest') {
        ...
        for (piece = next.to; piece >= next.from + next.offset; --piece) {
          debug('linear ' + wire.peerPieces.get(piece))
          if (!wire.peerPieces.get(piece)) continue
          if (self._request(wire, piece, false)) return
          debug('linear requested', piece)
        }
      }
    }

    debug('validated wire')
...
Torrent.prototype._request = function (wire, index, hotswap) {
  ...
  debug('request', index, block.offset, block.length)
